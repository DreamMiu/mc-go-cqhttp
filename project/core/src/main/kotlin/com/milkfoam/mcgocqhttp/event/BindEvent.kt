...
import com.milkfoam.mcgocqhttp.McGoCqHttp.expirationTime
    ...
    fun bind(data: BotData) {
        with(data) {
            ...
            if (message_type == "group") {
                ...
                //获取发送者的信息
                val userData = MGCAPI.getStrangerQQInfo(sendQQ)
                //获取发送者的QQ等级
                            ...
                            Bukkit.getPlayer(message.split(" ")[1])?.let { player ->
                                ...
                                if (!inBindList.containsValue(sendQQ)) {
                                    ...
                                    if (bindState == "未绑定" && !DataBaseManager.checkQQ(sendQQ)) {
                                        ...
                                        //发送绑定消息给QQ群
                                        GroupMessage().send(groupId, MGCAPI.atQQUser(sendQQ) + sendMessageToMinecraft)
                                        //监听玩家输入字符串并且30秒超时后发送超时消息给玩家
                                        player.nextChatInTick(expirationTime.toLong() * 20, { str ->
                                            //判断玩家游戏内输入的关键词
                                            if (str == bindKey) {
                                                //删除绑定状态
                                                ...
                                                //发送绑定成功消息给QQ群
                                                GroupMessage().send(groupId, MGCAPI.atQQUser(sendQQ) + success)
                                                //绑定成功执行指令
                                            ...
                                            //发送超时消息给QQ群
                                            GroupMessage().send(groupId, MGCAPI.atQQUser(sendQQ) + timeOut)
                                            //删除绑定状态
                                            inBindList.remove(player.uniqueId, sendQQ)
                                        })
                                    } else {
                                        ...
                                        //发送已经绑定消息给QQ群
                                        GroupMessage().send(groupId, MGCAPI.atQQUser(sendQQ) + isBind)
                                    }
                                } else {
                                    //QQ账号正在绑定中发送消息给QQ群
                                    GroupMessage().send(groupId, MGCAPI.atQQUser(sendQQ) + isInBind)
                                }
                            } ?: run {
                                //获取玩家失败发送消息给QQ群
                                GroupMessage().send(groupId, MGCAPI.atQQUser(sendQQ) + notFindPlayer)
                            }
                        } else {
                            //QQ账号未达到等级发送消息给QQ群
                            GroupMessage().send(groupId, MGCAPI.atQQUser(sendQQ) + levelIsNotStandard)
